...
try:
    ...
    print("ERROR: playwright library not found. Install with:")
    print("  pip install playwright")
    print("  playwright install chromium")
    sys.exit(1)

# Note: Using Playwright (Chromium) to bypass Windows TLS/SSL issues


def fetch_html_with_browser(url: str) -> str:
    """
    Fetch HTML using Playwright Chromium browser.
    This bypasses Python SSL/TLS and Windows Schannel issues completely.
    ...
    try:
        with sync_playwright() as p:
            ...
            if not html:
                raise Exception("Browser returned empty HTML")
...
class TikTokScraper:
    ...
    def scrape_profile(self, username: str) -> Dict[str, Any]:
        ...
        try:
            # Fetch the profile page using Playwright Chromium (bypasses Windows TLS issues)
            html_content = fetch_html_with_browser(profile_url)
            
        except Exception as e:
            error_msg = str(e).lower()
            # Check if it's a 404 error
            if '404' in error_msg or 'not found' in error_msg or 'status: 404' in error_msg:
                return {
